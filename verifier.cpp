...
char * verifier::createSetDir()
{
    ...
    sprintf(uniqueName,"%d%d%d%d%d%d%ld%d%d",Tm->tm_year,Tm->tm_mon,Tm->tm_mday,Tm->tm_hour,Tm->tm_min,Tm->tm_sec,(detail_time.tv_usec/1000),detail_time.tv_usec,Tm->tm_wday);
    sprintf(setDir,"%s//faces/%s", localdirname.c_str(),uniqueName);
...
verifier::verifier(const char *localusername)
{

    char *username=getenv("facetrainername");
    if(username==0)
    {
        printf("Using default data\n");
        username=new char [300];
        strcpy(username,localusername);
    }
    ...
    {
        printf("Add a new user to the database");
        if(!manager.saveSetting())
        {
            printf("Error saving data!\n");
            exit(-1);
        }
    }
    localdirname=manager.getBaseFloder()+manager.getUser(username);

    char init[300];
    sprintf(init,"%s/", localdirname.c_str());
    sprintf(facesDirectory,"%s//faces", localdirname.c_str());

    sprintf(modelDirectory,"%s//model", localdirname.c_str());
    sprintf(configDirectory,"%s//config", localdirname.c_str());
    char maceConfig[300];
...
void verifier::createBiometricModels(char* setName=NULL)
{
    ...
    for (int i=leftIndex;i<rightIndex;i++)
    {
        ...
        printf("Insert a set with lbp:%d\n",lbpAv->front());
...
void verifier::addFaceSet(IplImage **set,int size)
{
    ...
    char  dirName[300];
    sprintf(dirName,"%s//faces/%s", localdirname.c_str(),dirNameUnique);
